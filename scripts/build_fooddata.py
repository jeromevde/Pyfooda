#!/usr/bin/env python3
"""
Build pyfooda/data/fooddata.csv from raw USDA FoodData Central download.

Usage:
    1. Download the CSV bundle from https://fdc.nal.usda.gov/download-datasets
    2. Extract it somewhere, e.g. ~/Downloads/FoodData_Central_csv_2024-10-31
    3. Run: python scripts/build_fooddata.py /path/to/FoodData_Central_csv_...

The output is written to pyfooda/data/fooddata.csv and pyfooda/data/nutrients.csv.
"""
#%%
import sys
import pandas as pd
...
fooddata_folder = sys.argv[1] if len(sys.argv) > 1 else "/Users/jf41043/Downloads/FoodData_Central_csv_2024-10-31"
...
#%% --- SAVE ---
out_dir = os.path.join(os.path.dirname(__file__), '..', 'pyfooda', 'data')
os.makedirs(out_dir, exist_ok=True)

pivot_df.to_csv(os.path.join(out_dir, 'fooddata.csv'), index=False)
nutrient_df.to_csv(os.path.join(out_dir, 'nutrients.csv'))
# %%
